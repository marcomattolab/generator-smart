...
export class SpesaUpdatePage implements OnInit {
    ...
    ngOnInit() {
        this.trasfertaService.query()
            .subscribe(data => { this.trasfertas = data.body; }, (error) => this.onError(error));
    ...
    private createFromForm(): Spesa {
        return {
            ...
            dataSpesa: this.form.get(['dataSpesa']).value,
            stato: this.form.get(['stato']).value,
            approvedDate: this.form.get(['approvedDate']).value,
            trasfertaId: this.form.get(['trasfertaId']).value,
            structureId: this.form.get(['structureId']).value,
        };
    }
